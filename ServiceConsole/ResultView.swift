    ...
    func invokeRemoteFunction() {
        ...
        if let zcontext = self.zmqContext.get() {
            do {
                ...
                self.error = "Calling the remote command [\(command)] timed out."
            } catch SCError.proto(let command, let message) {
                self.error = "Contract violation while running [\(command)]: \(message)"
            } catch SCError.backend(let command, let message) {
                self.error = "Backend says [\(command)]: \(message)"
            } catch SCError.zmq(let command, let error) {
                self.error = "Communications failed [\(command)]: \(error.description)"
            } catch {
